...
const BestSeller = () => {
    ...
    const mainSellerStyle={display:"flex",justifyContent:"space-between",flexDirection:{xl:"row",lg:"row",md:"row",sm:"column",xs:"column"},flexWrap:{md:"wrap"},width:"100%",MaxHeight:"400px"}
    const cardSellStyle={display:"flex",flexDirection:"column",gap:2,width:{xl:"24%",md:"48%",sm:"100%",xs:"100%"},mb:{xs:"16px"},MaxHeight:"400px"}
    const sellTitle={fontSize:"18px",fontWeight:600,lineHeight:"21.33px",letterSpacing:"-0.24px"}
    const sellDesc={fontSize:"16px",fontWeight:400,lineHeight:"18.96px",letterSpacing:"-0.41px",color:"#BDBDBD",mb:"12px"}
    const sellPrice={fontSize:"12px",fontWeight:600,lineHeight:"14.22px",letterSpacing:"-0.24 px",color:"#00011F"}
    const addCartButton={width:"90%",height:"50px",bgcolor:"#9B1D08",borderRadius:"14px",padding:"10px",textAlign:"center",color:"#fff",textTransform:"none",":hover":{bgcolor:"#9B1D08"}}
  return (
    <Box sx={{mt:"126px"}}>
      <Container>
        <Box sx={headerStyle}>
        ...
        {items?.results?.map((product)=>(
            <Box key={product.id} sx={cardSellStyle}>
                <img src={product.image} height={"200px"}width={"250px"} alt='Best seller'/>
                <Typography sx={sellTitle}>{product.product_name}</Typography>
                <Typography sx={sellDesc}>{product.description.slice(0,100)}</Typography>
                <Typography sx={sellPrice}>{product.price}</Typography>
                <Button sx={addCartButton}>Add to cart</Button>
            </Box>
        ))}
        </Slider>
              {<SideCart />}
